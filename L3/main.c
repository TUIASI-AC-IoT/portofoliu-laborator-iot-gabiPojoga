...
static EventGroupHandle_t s_event_start_upgrade_version;
#define BIT_VERSION_PENDING BIT0

static const char *TAG = "simple_ota_example";
extern const uint8_t server_cert_pem_start[] asm("_binary_ca_cert_pem_start");
extern const uint8_t server_cert_pem_end[] asm("_binary_ca_cert_pem_end");

char received_version[256];
char received_version_len = 0;
int version_id = 0;
...
esp_err_t _http_event_handler_version(esp_http_client_event_t *evt)
{
    switch (evt->event_id) {
        ...
        case HTTP_EVENT_ON_HEADER:
            ...
            break;
        case HTTP_EVENT_ON_DATA:
            received_version_len = evt->data_len;
            strncpy(received_version, (char *) evt->user_data, received_version_len);
            version_id = atoi(received_version);
            xEventGroupSetBits(s_event_start_upgrade_version, BIT_VERSION_PENDING);
            ESP_LOGI(TAG, "VERSION ID RECEIVED = %d", version_id);
            ESP_LOGI(TAG, "HTTP_EVENT_ON_DATA, len=%d", evt->data_len);
            break;
        case HTTP_EVENT_ON_FINISH:
            ESP_LOGI(TAG, "HTTP_EVENT_ON_FINISH");
...
static void ota_task(void *pvParameters)
{
    ...
    esp_http_client_config_t request_config = {
        .url = CONFIG_CHECK_FIRMWARE_UPGRADE_URL,
        .cert_pem = (char *)server_cert_pem_start,
        .cert_len = 1422,
        .event_handler = _http_event_handler_version,
        .user_data = received_version,
        .keep_alive_enable = true,
        .use_global_ca_store = true,
        .skip_cert_common_name_check = true
    };

    esp_http_client_handle_t request_client = esp_http_client_init(&request_config);
    ...
    ESP_LOGI(TAG, "Attempting to download update from %s", request_config.url);
    
    esp_err_t err = esp_http_client_perform(request_client);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Version received");
        esp_restart();
    } else {
        ESP_LOGE(TAG, "Version receiving failed...");
    }
    while (1) {
        vTaskDelay(1000 / portTICK_PERIOD_MS);
    }

    int current_version = atoi(BUILD_NUMBER);

    xEventGroupWaitBits(s_event_start_upgrade_version, BIT_VERSION_PENDING, pdTRUE, pdTRUE, portMAX_DELAY);
    if (version_id < current_version) {
        ...
        ESP_LOGI(TAG, "There is no newer version...");
    }
}
...
void app_main(void)
{
    ...
    if (connected) {
        ...
        s_event_start_upgrade_version = xEventGroupCreate();
